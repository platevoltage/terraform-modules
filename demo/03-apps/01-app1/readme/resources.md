...
|-----|-----|-------------|
| aws_cloudwatch_log_group.fargate_task_log_group | resource | KMS encrypted CloudWatch log group for application logs (inside ../../../modules/ecs-service) |
| aws_cloudwatch_metric_alarm.unhealthy_instance_count | resource | Alarm triggered when ALB target group reports unhealthy tasks (inside ../../../modules/tg-fargate) |
...
| aws_codedeploy_app.ecs | resource | CodeDeploy application for ECS deployments (inside ../../../modules/ecs-service) |
| aws_codedeploy_deployment_group.ecs | resource | CodeDeploy deployment group managing traffic shifting (inside ../../../modules/ecs-service) |
...
| aws_ecs_service.ecs_app_service_codedeploy | resource | ECS service controlled by CodeDeploy for blue green deployments (inside ../../../modules/ecs-service) |
| aws_ecs_service.ecs_app_service_rolling | resource | } ECS service used for rolling deployments (inside ../../../modules/ecs-service) |
| aws_ecs_task_definition.app | resource | ECS task definition for the Hello World application (inside ../../../modules/ecs-service) |
| aws_iam_policy.ecs_exec_policy | resource | Policy enabling ECS Exec support (inside ../../../modules/ecs-service) |
| aws_iam_policy.secrets_manager_policy | resource | Optional policy allowing Secrets Manager access (inside ../../../modules/ecs-service) |
| aws_iam_policy.sqs_policy | resource | Optional policy allowing SQS access (inside ../../../modules/ecs-service) |
...
| aws_iam_role.ecs_task_role | resource | ###################### IAM Role for the Task ###################### IAM role assumed by the ECS task (inside ../../../modules/ecs-service) |
...
| aws_iam_role_policy_attachment.codedeploy_managed | resource | Attaches AWS managed CodeDeploy ECS policy (inside ../../../modules/ecs-service) |
| aws_iam_role_policy_attachment.ecs_exec_policy_attachment | resource | Attaches ECS Exec policy to the task role (inside ../../../modules/ecs-service) |
| aws_iam_role_policy_attachment.secrets_manager_policy_attachment | resource | Attaches Secrets Manager policy to the task role (inside ../../../modules/ecs-service) |
| aws_iam_role_policy_attachment.sqs_policy_attachment | resource | Attaches SQS policy to the task role (inside ../../../modules/ecs-service) |
| aws_kms_alias.cloudwatch_logs | resource | Alias for the CloudWatch Logs KMS key (inside ../../../modules/ecs-service) |
...
| aws_security_group.ecs_fargate_task | resource | Security group attached to the Fargate task (inside ../../../modules/ecs-service) |
| aws_security_group_rule.ecs_fargate_task_egress | resource | Allows outbound IPv4 traffic from the ECS task (inside ../../../modules/ecs-service) |
| aws_security_group_rule.ecs_fargate_task_egress_v6 | resource | Allows outbound IPv6 traffic from the ECS task (inside ../../../modules/ecs-service) |
| aws_security_group_rule.from_alb_to_task | resource | Allows ALB to reach the application container port (inside ../../../modules/ecs-service) |
...
| data.aws_iam_policy_document.codedeploy_assume | data source | IAM role assumed by CodeDeploy (inside ../../../modules/ecs-service) |
...
| data.aws_iam_policy_document.sns_topic_policy | data source | Allows CodeStar Notifications to publish to the application SNS topic |
